...
#include "ena_com.h"
#include <dev/enav2/base/ena_defs/ena_admin_defs.h>
			      ...
			      struct ena_com_io_sq *io_sq)
{
	...
	if (io_sq->mem_queue_type == ENA_ADMIN_PLACEMENT_POLICY_DEV) {
		...
		if (!io_sq->bounce_buf_ctrl.base_buffer)
			io_sq->bounce_buf_ctrl.base_buffer = static_cast<u8*>(ENA_MEM_ALLOC(ena_dev->dmadev, size));
					   ...
					   u16 log_size)
{
	...
	rss->host_rss_ind_tbl =
		static_cast<u16*>(ENA_MEM_ALLOC(ena_dev->dmadev, tbl_size));
			      ...
			      enum ena_admin_hash_functions *func)
{
	...
	rss->hash_func = static_cast<enum ena_admin_hash_functions>(ENA_FFS(get_resp.u.flow_hash_func.selected_func));
	if (rss->hash_func)
      rss->hash_func = static_cast<enum ena_admin_hash_functions>(static_cast<int>(rss->hash_func) - 1);
...
int ena_com_set_default_hash_ctrl(struct ena_com_dev *ena_dev)
{
	...
	rc = ena_com_get_hash_ctrl(ena_dev, static_cast<enum ena_admin_flow_hash_proto>(0), NULL);
	if (unlikely(rc))
		return rc;
	...
	if (unlikely(rc))
		ena_com_get_hash_ctrl(ena_dev, static_cast<enum ena_admin_flow_hash_proto>(0), NULL);
