...
{
	if (desc) {
		[[maybe_unused]] uint32_t *desc_arr = (uint32_t *)desc;
...
static void ena_com_rx_set_flags(struct ena_com_rx_ctx *ena_rx_ctx,
				 struct ena_eth_io_rx_cdesc_base *cdesc)
{
	ena_rx_ctx->l3_proto = static_cast<enum ena_eth_io_l3_proto_index>(cdesc->status &
		       ...
		       int *nb_hw_desc)
{
	...
	rc = ena_com_write_header_to_bounce(io_sq, static_cast<u8*>(buffer_to_push), header_len);
	if (unlikely(rc))
		return rc;
