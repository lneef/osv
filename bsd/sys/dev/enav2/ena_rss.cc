...
void ena_rss_key_fill(void *key, size_t size)
{
	...
	memcpy(key, default_key, RTE_MIN(size, sizeof(default_key)));
}

int ena_rss_reta_update(rte_eth_dev *dev,
			...
			uint16_t reta_size)
{
	...
	rte_spinlock_unlock(&adapter->admin_lock);
	if (unlikely(rc != 0)) {
		ena_log_raw(ERR, "Cannot set the indirection table");
		return rc;
	}

	ena_log_raw(DBG, "RSS configured %d entries for port %d",
			    ...
			    bool default_allowed)
{
	...
	if (rc == ENA_COM_UNSUPPORTED) {
		...
		ena_log_raw(ERR, "Failed to set RSS hash fields");
		return rc;
	}
...
int ena_rss_configure(struct ena_adapter *adapter)
{
	...
	rss_conf = &adapter->edev->data.dev_conf.rx_adv_conf.rss_conf;
	...
	ena_log_raw(DBG, "RSS configured for port %d",
		adapter->edev->data.port_id);
