...
u8 requested_options[] = {
    dhcp::DHCP_OPTION_SUBNET_MASK,         dhcp::DHCP_OPTION_ROUTER,
    dhcp::DHCP_OPTION_DOMAIN_NAME_SERVERS, dhcp::DHCP_OPTION_INTERFACE_MTU,
    dhcp::DHCP_OPTION_BROADCAST_ADDRESS,   dhcp::DHCP_OPTION_HOSTNAME};

const ip::address_v4 ipv4_zero =
    ip::address_v4::address_v4::from_string("0.0.0.0");
...
void dhcp_release() { net_dhcp_worker.release(); }

void dhcp_renew(bool wait) { net_dhcp_worker.renew(wait); }
...
void dhcp_mbuf::detach() { _attached = false; }

struct mbuf *dhcp_mbuf::get() { return _m; }

void dhcp_mbuf::set(struct mbuf *m) { _m = m; }
...
void dhcp_mbuf::compose_request(struct ifnet *ifp, u32 xid, ip::address_v4 yip,
                                ip::address_v4 sip,
  ...
  *pkt = {};
  ...
  if (request_packet_type == DHCP_REQUEST_SELECTING) {
    options =
        add_option(options, DHCP_OPTION_DHCP_SERVER, 4, (u8 *)&dhcp_server_ip);
  }
  if (hostname.length() > 0) {
    options = add_option(options, DHCP_OPTION_HOSTNAME, hostname.length(),
                         (u8 *)(hostname.c_str()));
  }
...
void dhcp_mbuf::compose_release(struct ifnet *ifp, ip::address_v4 yip,
                                ip::address_v4 sip) {
  size_t dhcp_len = sizeof(struct dhcp_packet);
  struct dhcp_packet *pkt = pdhcp();
  *pkt = {};
...
u32 dhcp_mbuf::get_xid() { return (pdhcp()->xid); }
...
bool dhcp_mbuf::decode() {
  ...
  while (options < limit) {
    ...
    case DHCP_OPTION_SUBNET_MASK:
      PARSE_OP(ip::address_v4::bytes_type, ip::address_v4::bytes_type, bytes);
    ...
    case DHCP_OPTION_CLASSLESS_ROUTE:
      while (op_len) {
        ...
        _routes.emplace_back(
            ip::address_v4(ntohl(net)),
            ip::address_v4(u32(((1ull << mask) - 1) << (32 - mask))),
            ip::address_v4(bytes));
      }
      break;
  ...
  dhcp_i("Received %s message from DHCP server: %s regarding offerred IP "
         "address: %s",
         dhcp_message_type_name_by_id[_message_type],
         _dhcp_server_ip.to_string().c_str(), _your_ip.to_string().c_str());
  return true;
}

struct ip *dhcp_mbuf::pip() { return mtod(_m, struct ip *); }
...
struct dhcp_packet *dhcp_mbuf::pdhcp() {
  return reinterpret_cast<struct dhcp_packet *>(mtod(_m, u8 *) + _ip_len +
                                                udp_len);
}

u8 *dhcp_mbuf::poptions() { return (reinterpret_cast<u8 *>(pdhcp() + 1)); }
...
void dhcp_mbuf::build_udp_ip_headers(size_t dhcp_len, in_addr_t src_addr,
  ...
  // Set length in mbuf
  _m->M_dat.MH.MH_pkthdr.len = _m->m_hdr.mh_len =
      min_ip_len + udp_len + dhcp_len;
...
dhcp_interface_state::~dhcp_interface_state() { delete _sock; }
...
void dhcp_interface_state::release() {
  ...
  // Save transaction id & send
  _xid = dm.get_xid();
  dhcp_i("Unicasting DHCPRELEASE message with xid: [%d] from client: %s to "
         "server: %s",
...
void dhcp_interface_state::renew() {
  ...
  dm.compose_request(_ifp, _xid, _client_addr, _server_addr,
                     dhcp_mbuf::DHCP_REQUEST_RENEWING, hostname_str);

  // Send
  dhcp_i("Unicasting DHCPREQUEST message with xid: [%d] from client: %s to "
         "server: %s in order to RENEW lease of: %s",
         _xid, _client_addr.to_string().c_str(),
...
void dhcp_interface_state::state_discover(dhcp_mbuf &dm) {
  if (dm.get_message_type() != DHCP_MT_OFFER) {
    dhcp_w("Not offer packet in discover state, type = %d",
           dm.get_message_type());
    return;
  }
  ...
  dm_req.compose_request(_ifp, _xid, dm.get_your_ip(), dm.get_dhcp_server_ip(),
                         dhcp_mbuf::DHCP_REQUEST_SELECTING, hostname_str);
  dhcp_i("Broadcasting DHCPREQUEST message with xid: [%d] to SELECT offered "
         "IP: %s",
...
void dhcp_interface_state::state_request(dhcp_mbuf &dm) {
  if (dm.get_message_type() == DHCP_MT_ACK) {
    dhcp_i("Server acknowledged IP %s for interface %s with time to lease in "
           "seconds: %d",
    ...
    printf("%s: %s\n", _ifp->if_xname, dm.get_your_ip().to_string().c_str());
    ...
    osv::start_if(_ifp->if_xname, dm.get_your_ip().to_string().c_str(),
                  dm.get_subnet_mask().to_string().c_str());

    if (dm.get_subnet_mask() == ip::address_v4({0xff, 0xff, 0xff, 0xff})) {
      osv_route_add_interface(dm.get_router_ip().to_string().c_str(), nullptr,
                              _ifp->if_xname);
    }
    osv_route_add_network("0.0.0.0", "0.0.0.0",
                          dm.get_router_ip().to_string().c_str());

    std::for_each(dm.get_routes().begin(), dm.get_routes().end(),
                  [&](dhcp_mbuf::route_type &r) {
                    auto dst = std::get<0>(r);
                    auto mask = std::get<1>(r);
                    auto gw = std::get<2>(r);

                    dhcp_i("adding route: %s/%s -> %s", dst.to_string().c_str(),
                           mask.to_string().c_str(), gw.to_string().c_str());

                    if (gw == ip::address_v4::any()) {
                      osv_route_add_interface(dst.to_string().c_str(),
                                              mask.to_string().c_str(),
                                              _ifp->if_xname);
                    } else {
                      osv_route_add_network(dst.to_string().c_str(),
                                            mask.to_string().c_str(),
                                            gw.to_string().c_str());
                    }
                  });
...
void dhcp_worker::init() {
  ...
  TAILQ_FOREACH(ifp, &V_ifnet, if_link) {
    if ((!(ifp->if_flags & IFF_DYING)) && (!(ifp->if_flags & IFF_LOOPBACK))) {
      _universe.insert(std::make_pair(ifp, new dhcp_interface_state(ifp)));
    }
  }
  IFNET_RUNLOCK();
...
void dhcp_worker::dhcp_worker_fn() {
  while (true) {
    mbuf *m;
    WITH_LOCK(_lock) {
      sched::thread::wait_until(_lock, [&] { return (!_rx_packets.empty()); });
...
void dhcp_worker::queue_packet(struct mbuf *m) {
  ...
  WITH_LOCK(_lock) { _rx_packets.push_front(m); }
...
void dhcp_buf::compose_discover(rte_ether_addr &addr) {

  size_t dhcp_len = sizeof(dhcp::dhcp_packet);
  dhcp::dhcp_packet *pkt = pdhcp();
  *pkt = {};

  // Header
  srand((unsigned int)osv::clock::wall::now().time_since_epoch().count());
  pkt->op = dhcp::BOOTREQUEST;
  pkt->htype = dhcp::HTYPE_ETHERNET;
  pkt->hlen = RTE_ETHER_ADDR_LEN;
  pkt->hops = 0;
  pkt->xid = rand();
  pkt->secs = 0;
  pkt->flags = 0;
  memcpy(pkt->chaddr, addr.addr.data(), ETHER_ADDR_LEN);
  ...
  options = add_option(options, dhcp::DHCP_OPTION_MESSAGE_TYPE, 1,
                       dhcp::DHCP_MT_DISCOVER);
  options = add_option(options, dhcp::DHCP_OPTION_PARAMETER_REQUEST_LIST,
                       sizeof(requested_options), requested_options);
  *options++ = dhcp::DHCP_OPTION_END;

  dhcp_len += options - options_start;
  build_udp_ip_headers(dhcp_len, INADDR_ANY, INADDR_BROADCAST);
  build_eth_ip_headers(addr, rte_ether_addr::broadcast);
}

void dhcp_buf::compose_request(rte_ether_addr &addr, u32 xid,
                               ip::address_v4 yip, ip::address_v4 sip,
                               dhcp_request_packet_type request_packet_type,
                               std::string hostname) {
  size_t dhcp_len = sizeof(struct dhcp_packet);
  struct dhcp_packet *pkt = pdhcp();
  *pkt = {};
  ...
  pkt->secs = 0;
  pkt->flags = 0;
  memcpy(pkt->chaddr, addr.addr.data(), RTE_ETHER_ADDR_LEN);
  ulong yip_n = htonl(yip.to_ulong());
  ulong sip_n = htonl(sip.to_ulong());
  if (request_packet_type == DHCP_REQUEST_RENEWING ||
      request_packet_type == DHCP_REQUEST_REBINDING) {
...
bool dhcp_buf::is_valid_dhcp(){
    decode_ip_len();
  struct ip *ip = pip();
  struct udphdr *udp = pudp();
  u8 *options = poptions();

  if (_m->pkt_len < static_cast<uint32_t>(_ip_len + dhcp::udp_len + dhcp::min_dhcp_len)) {
    return false;
  }
