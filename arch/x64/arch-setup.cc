...
void for_each_e820_entry(void *e820_buffer, unsigned size,
                         void (*f)(e820ent e)) {
  auto p = e820_buffer;
...
// Because vmlinux_entry64 replaces start32 as a new entry of loader.elf we need
// a way to place address of start32 so that boot16 know where to jump to. We
// achieve it by placing address of start32 at the known offset at memory as
// defined by section .start32_address in loader.ld
...
void *__attribute__((
    section(".start32_from_vmlinuz_address"))) start32_from_vmlinuz_address =
...
void arch_setup_free_memory() {
  ...
  for_each_e820_entry(e820_buffer, e820_size,
                      [](e820ent ent) { memory::phys_mem_size += ent.size; });
...
void init_cache_modes() {
  auto pat_val_msr = mmu::init_pat();
  processor::wrmsr(msr::IA32_PAT, pat_val_msr);
  mmu::flush_tlb_local();
}
void init_syscall() {
  ...
  processor::wrmsr(msr::IA32_EFER,
                   processor::rdmsr(msr::IA32_EFER) | IA32_EFER_SCE);
}
} // namespace processor
...
void arch_init_early_console() { console::isa_serial_console::early_init(); }
