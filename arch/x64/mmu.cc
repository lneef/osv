...
void flush_tlb_all() {
  ...
  if (sched::thread::current()->is_app()) {
    ipis.clear();
    std::copy_if(
        sched::cpus.begin(), sched::cpus.end(), std::back_inserter(ipis),
        [](sched::cpu *c) {
...
bool is_page_fault_prot_write(unsigned int error_code) {
  return (error_code & (page_fault_write | page_fault_prot)) ==
         (page_fault_write | page_fault_prot);
}
...
// unified data and instruction caches. Therefore we do not need to do anything
// as they are always in sync.
